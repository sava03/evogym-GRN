...
class GRN:
    ...
    def __init__(self, promoter_threshold=0.95, max_voxels=27, cube_face_size=3,
        ...
        # evolved controller params: genome[2..4] mapped to physical ranges
        self.controller_bias      = 0.8 + float(self.genotype[2]) * 0.4   # [0.8, 1.2]
        self.controller_amplitude = 0.1 + float(self.genotype[3]) * 0.5   # [0.1, 0.6]
        self.controller_period    = int(10 + float(self.genotype[4]) * 30) # [10, 40] steps

        self.genotype = self.genotype[5:]
...
def initialization(rng, ini_genome_size):
    ...
    # +1 concentration, +1 offphase alternation, +3 controller (bias/amplitude/period)
    genome_size = genome_ini_size + 5
