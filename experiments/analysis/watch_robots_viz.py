...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Optional CSV generation filter, e.g. '10,20,50'. Omit for best-ever across all generations.",
    )
    p.add_argument(
        "--runs",
        type=str,
        default=None,
        help="CSV run filter overriding the params file, e.g. '2' or '1,3'.",
    )
    p.add_argument(
        "--max_voxels",
        type=int,
        default=None,
        help="Override max_voxels from params file.",
    )
    p.add_argument(
        "--cube_face_size",
        type=int,
        default=None,
        help="Override cube_face_size from params file.",
    )
    return p.parse_args()
...
def _win_to_bash_path(path: Path, bash_exe: str) -> str:
    s = str(path).replace("\\", "/")
    if len(s) >= 2 and s[1] == ":":
        prefix = "/mnt/" if "system32" in bash_exe.lower() or "windowsapps" in bash_exe.lower() else "/"
        s = prefix + s[0].lower() + s[2:]
    return s
...
def _load_params_from_shell(params_path: Path):
    ...
    bash_exe = _get_bash_exe()
    bash_path = _win_to_bash_path(params_path, bash_exe)
    printf_cmds = "; ".join(
        f"printf '{k}=%s\\n' \"${k}\"" for k in PARAM_KEYS
    )
    cmd = f"source {shlex.quote(bash_path)}; {printf_cmds}"
    proc = subprocess.run(
        [bash_exe, "-lc", cmd],
        check=True,
        text=True,
...
def main():
    ...
    runs = [int(x) for x in _split_csv(args.runs or params.get("runs"))]
    ...
    for rank, entry in enumerate(selected, start=1):
        ...
        max_voxels = args.max_voxels if args.max_voxels is not None else int(params.get("max_voxels") or 64)
        cube_face_size = args.cube_face_size if args.cube_face_size is not None else int(params.get("cube_face_size") or 4)
